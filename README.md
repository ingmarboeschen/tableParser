...
The textual representation of characters in matrix content can be unified with *unifyMatrix()* before parsing. The function *table2stats()* extracts tabled statistical results. The function further unifies the parsed text, which is then processed with <a href="https://CRAN.R-project.org/package=tableParser/" target="_blank"  rel="noopener noreferrer">JATSdecoder</a>'s function standardStats(), in order to extract all statistical standard results (t, Z, χ2, F, r, d, β, SE, r, d, η2, ω2, OR, RR, p), the coded p-values and recompute p-values if possible ([Böschen, 2021](https://doi.org/10.1038/s41598-021-98782-3)). 
...
Example matrix with footnotes extracted from file with:
...
Collapsed matrix content with decoded p-values and degrees of freedom special handling, retrieved with:
